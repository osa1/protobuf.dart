  ...
  void writeField(int fieldNumber, int fieldType, fieldValue) {
    ...
    if ((fieldType & PbFieldType.MAP_BIT) != 0) {
      ...
      fieldValue.forEach((key, value) {
        ...
        final mark = _startLengthDelimited();
        _writeValue(
            PbMap.keyFieldNumber, fieldValue.keyFieldType, key, keyWireFormat);
        _writeValue(PbMap.valueFieldNumber, fieldValue.valueFieldType, value,
            valueWireFormat);
        _endLengthDelimited(mark);
      });
      return;
  ...
  void _writeValueAs(int valueType, dynamic value) {
    switch (valueType) {
      ...
      case PbFieldType.STRING_BIT:
        _writeBytesNoTag(utf8.encode(value));
        break;
