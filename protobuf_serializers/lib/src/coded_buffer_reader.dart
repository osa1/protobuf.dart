...
class CodedBufferReader {
  ...
  void _mergeMessage(
      GeneratedMessage msg, ExtensionRegistry extensionRegistry) {
    final info = msg.fieldSet.meta;
    mergeFromCodedBufferReader(info, msg.fieldSet, this, extensionRegistry);
  }

  void _mergeUnknownFieldSet(UnknownFieldSet unknownFieldSet) {
    unknownFieldSet.ensureWritable('mergeFromCodedBufferReader');
    while (true) {
      var tag = readTag();
      if (tag == 0 || !_mergeField(tag, unknownFieldSet)) {
        break;
      }
    }
  }

  bool _mergeField(int tag, UnknownFieldSet unknownFieldSet) {
    unknownFieldSet.ensureWritable('_mergeField');
  ...
  void readGroup(int fieldNumber, GeneratedMessage message,
    ...
    ++_recursionDepth;
    _mergeMessage(message, extensionRegistry);
  ...
  UnknownFieldSet readUnknownFieldSetGroup(int fieldNumber) {
    ...
    _mergeUnknownFieldSet(unknownFieldSet);
    checkLastTagWas(makeTag(fieldNumber, WIRETYPE_END_GROUP));
    --_recursionDepth;
